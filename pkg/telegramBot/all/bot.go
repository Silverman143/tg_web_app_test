...
func (bot *BotAPI) MakeRequest(endpoint string, params url.Values) (APIResponse, error) {
	...
	logrus.Info("________________________"+method)
...
func (bot *BotAPI) CreateInvoiceLink(config InvoiceLinkConfig) (APIResponse, error) {
	v, err := config.values()
	if err != nil {
		return APIResponse{}, err
	}
...
func (bot *BotAPI) AnswerPreCheckout(config PreCheckoutAnswerConfig) (APIResponse, error) {
	v, err := config.values()
	if err != nil {
		logrus.Errorf("Error with config", err)
		return APIResponse{}, err
	}
	bot.debugLog(config.method(), v, nil)

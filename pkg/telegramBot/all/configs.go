...
func (config InvoiceLinkConfig) values() (url.Values, error) {
	v := url.Values{} 
...
// PreCheckoutAnswerConfig представляет конфигурацию для ответа на pre-checkout query
type PreCheckoutAnswerConfig struct {
	PreCheckoutQueryID string // обязательное поле
	OK                 bool   // обязательное поле
	ErrorMessage       string // поле по умолчанию
}

// method возвращает метод API для ответа на pre-checkout query
func (config PreCheckoutAnswerConfig) method() string {
	return "answerPreCheckoutQuery"
}

// values преобразует конфигурацию в url.Values для использования в запросе
func (config PreCheckoutAnswerConfig) values() (url.Values, error) {
	v := url.Values{}
